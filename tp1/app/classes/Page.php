...
class Page
{
    ...
    // Fonction d'insertion des utilisateurs lors de la création de compte
    ...
    // Fonction d'insertion des clients lors de la création d'une intervention
    ...
    public function insertIntervention(string $table_name, array $data)
    {
        $sql = "INSERT INTO " . $table_name . " (id_client, id_standariste, id_statut, id_degre, id_commentaire,
                description, type, date, heure) VALUES(:id_client, :id_standardiste, :id_statut, :id_degre, :id_commentaire,
                :description, :type, :date, :heure)"; 
         $stmt = $this->pdo->prepare($sql); // Préparation de la requête SQL
        $stmt->execute($data); // Exécution de la requête SQL avec les données fournies
    }
    ...
    // Fonction qui récupère les utilisateurs qui ont pour rôle intervenant
